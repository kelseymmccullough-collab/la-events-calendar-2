...
def scrape_vidiots():
    ...
    try:
        ...
        for header in all_headers:
            ...
            # Get the parent card - go up 2 levels
            parent = header.find_parent()
            if not parent:
                continue
            ...
            # --- Use structured data-date elements to pair dates with showtimes ---
            # 1. Build date map from <li class="show-date" data-date="..."> elements
            date_lis = parent.find_all('li', class_='show-date')
            date_map = {}  # data-date value -> parsed date string
            for dli in date_lis:
                data_date = dli.get('data-date', '')
                date_text = dli.get_text(strip=True)
                # Parse "Thu, Feb 19" or "Fri,  Feb 20" etc.
                dm = re.search(r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+(\d{1,2})', date_text, re.I)
                if dm and data_date:
                    month_num = month_abbr_map.get(dm.group(1)[:3].capitalize(), 0)
                    day = int(dm.group(2))
                    if month_num:
                        date_map[data_date] = f"{current_year}-{month_num:02d}-{day:02d}"
            
            # 2. Find all showtime <li data-date="..."> that are NOT date selectors
            showtime_lis = [
                li for li in parent.find_all('li', attrs={'data-date': True})
                if 'show-date' not in (li.get('class') or [])
            ]
            
            if showtime_lis and date_map:
                # Structured approach: pair each showtime with its date via data-date
                for st_li in showtime_lis:
                    data_date = st_li.get('data-date', '')
                    date_str = date_map.get(data_date)
                    if not date_str:
                        continue
                    
                    # Extract time from the showtime li text (e.g., "7:00 pmLimited Walk-Ups*")
                    st_text = st_li.get_text(strip=True)
                    time_match = re.search(r'(\d{1,2}:\d{2}\s*(?:am|pm))', st_text, re.I)
                    if not time_match:
                        continue
                    time_str = time_match.group(1).strip().upper()
                    
                    # Get the purchase URL if available
                    event_url = default_url
                    purchase_link = st_li.find('a', class_='showtime')
                    if purchase_link and purchase_link.get('href'):
                ...
                # Fallback: use text-based parsing for films without structured data
                section_text = parent.get_text()
                ...
                month_num = month_abbr_map.get(date_match.group(1)[:3].capitalize(), 0)
                day = int(date_match.group(2))
                if not month_num:
                    continue
                date_str = f"{current_year}-{month_num:02d}-{day:02d}"
...
def scrape_american_cinematheque():
    ...
    try:
        ...
        while page_num <= max_pages:
            ...
            # Use mobileEventCalendar__event divs - each one is a clean event card
            # with structured spans for date, time, title, and a link for the URL
            event_cards = soup.find_all('div', class_='mobileEventCalendar__event')
            ...
            for card in event_cards:
                try:
                    ...
                    # Extract time from span.eventTime (e.g., "10:00 pm")
                    time_el = card.find('span', class_='eventTime')
                    time_text = time_el.get_text(strip=True) if time_el else None
                    
                    # Extract URL from "View Event Details" link
                    detail_link = card.find('a', href=lambda h: h and '/now-showing/' in h and h != '/now-showing/')
                    ...
                    # Parse date from the eventDate span (e.g., "Sat, February 21, 2026")
                    date_str = None
                    if date_text:
                    ...
                    # Fallback: parse date from URL if not found in span
                    ...
                    # Parse time from the eventTime span (e.g., "10:00 pm")
                    time_str = None
                    if time_text:
                    ...
                    # Fallback: parse time from URL if not found in span
                    if not time_str and href:
                        url_time_match = re.search(r'-(\d{1,4})(am|pm)/?$', href, re.I)
                        if url_time_match:
                            time_num = url_time_match.group(1)
                            period = url_time_match.group(2).upper()
                            if len(time_num) <= 2:
                                hour = int(time_num)
                                minutes = "00"
                            elif len(time_num) == 3:
                    ...
                    if not time_str:
                        time_str = "7:00 PM"  # Default fallback
                    
                    # If no title from span, try to extract from URL slug
